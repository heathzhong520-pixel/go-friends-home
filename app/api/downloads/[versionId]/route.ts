...
export async function GET(request: Request, context: { params: Promise<{ versionId: string }> }) {
  ...
  try {
    ...
    if (!item || !item.version.ossObjectKey) return json({ error: copy.packageMissing }, { status: 404 });
    const user = await getUserFromRequest(request);
    let licenseId: string | null = null;
    if (!item.product.isFree) {
      if (!user) return json({ error: copy.unauthorized }, { status: 401 });
      ...
      if (!license) return json({ error: copy.licenseMissing }, { status: 403 });
