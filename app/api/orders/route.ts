...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (input.paymentMethod === "test" && process.env.PAYMENT_TEST_MODE !== "true") return json({ error: copy.testDisabled }, { status: 403 });
    const db = getDb();
    const [product] = await db.select().from(products).where(and(eq(products.id, input.productId), eq(products.status, "active"))).limit(1);
    if (!product) return json({ error: copy.productUnavailable }, { status: 404 });
    const now = new Date();
